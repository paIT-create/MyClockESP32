...
<!-- Styl CSS dla ładnego układu próbników (wklej do sekcji <style> jeśli chcesz) -->
.row-color { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
.row-color input[type="color"] { border: none; width: 45px; height: 35px; border-radius: 6px; cursor: pointer; background: none; }
...
let nEnd = "6";
let currentID = "----";
var isUserInteracting = false; // Flaga blokująca odświeżanie podczas klikania
...
function loadStatus(){
  // Jeśli użytkownik właśnie kręci suwakiem lub wybiera kolor, 
  // ignorujemy pobieranie statusu, aby nic nie mignęło ani się nie nadpisało!
  ...
  fetch('/status?t=' + Date.now()).then(r=>r.text()).then(t=>{
    ...
    lines.forEach(l => {
      ...
      // --- PARSOWANIE STATUSU RGB ---
      if (key === "ver") {
        // Inteligentny bezpiecznik: Jeśli w wersji oprogramowania nie ma słowa NEOPIXEL,
        // strona automatycznie i całkowicie ukrywa kartę ustawień kolorów RGB!
        if (!val.includes("NEOPIXEL")) {
          document.getElementById("rgb-card").style.display = "none";
        } else {
          document.getElementById("rgb-card").style.display = "block";
        }
      }
      if (key === "effect") {
        document.getElementById("fx-select").value = val;
        // Jeśli włączona pełna tęcza przestrzenna, ukrywamy zbędne próbniki kolorów
        document.getElementById("static-colors-grp").style.display = (val === "2") ? "none" : "block";
      }
      if (key === "hcolor") document.getElementById("picker-hm").value = val;
      if (key === "scolor") document.getElementById("picker-sec").value = val;
      if (key === "ccolor") document.getElementById("picker-colon").value = val;
      if (key === "tcolor") document.getElementById("picker-temp").value = val;
      if (key === "acolor") document.getElementById("picker-ambient").value = val;
      // --- OBSŁUGA CHECKBOXA AUTOMATYKI TEMPERATURY ---
      if (key === "tAuto") {
        let chk = document.getElementById("chk-tauto");
        let pick = document.getElementById("picker-temp");
        if(chk) chk.checked = (val === "1");
        if(pick) pick.disabled = (val === "1"); // Blokuje próbnik, gdy działa automat LDR
      }
      ...
      const hide = ["id", "rssi", "ver", "nStart", "nEnd", "night", "day", "effect", "tAuto", "hcolor", "scolor", "ccolor", "tcolor", "acolor"];
...
function freezeStatus(shouldFreeze) {
  if (shouldFreeze) {
    isUserInteracting = true;
    // Opcjonalnie usuwamy ewentualny stoper, jeśli istniał
    if (interactionTimeout) clearTimeout(interactionTimeout);
  } else {
    // Dajemy bezpieczne 500ms zwłoki po zamknięciu okna, 
    // aby ESP32 zdążyło przetrawić ostatni wysłany pakiet przed odebraniem statusu
    interactionTimeout = setTimeout(function() {
      isUserInteracting = false;
    }, 500);
  }
}
function sendRGBParam(param, value) {
  // Dla listy rozwijanej (efektów) i checkboxów stosujemy krótkie zamrożenie
  isUserInteracting = true;
  clearTimeout(interactionTimeout);
  interactionTimeout = setTimeout(function() { isUserInteracting = false; }, 1500);

  fetch('/set?' + param + '=' + value).then(response => {
    if(param === 'effect') {
      document.getElementById("static-colors-grp").style.display = (value === "2") ? "none" : "block";
    }
    if(param === 'tAuto') {
      document.getElementById("picker-temp").disabled = (value === '1');
    }
  });
}
function sendRGBHex(param, hexValue) {
  // Funkcja wysyła dane w locie przy każdym ruchu suwaka (oninput) [INDEX]
  // Ponieważ onfocus zablokował loadStatus, nic nam nie nadpisze suwaka! [INDEX]
